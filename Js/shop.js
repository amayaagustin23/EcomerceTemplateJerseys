...
if (screen.width < 1220) {
  ...
  document
    ...
    .classList.add("collapse");
  document
    .getElementById("panelsStayOpen-collapseThree")
    .classList.add("collapse");
}
const LabelColor = () => {
  let etiquetas = document.getElementsByClassName("etiqueta");
  for (let index = 0; index < etiquetas.length; index++) {
    if (etiquetas[index].innerHTML === "OFERTA")
      etiquetas[index].style.backgroundColor = "green";
    if (etiquetas[index].innerHTML === "NUEVO")
      etiquetas[index].style.backgroundColor = "orange";
  }
};
...
const FiltrosSearch = () => {
  ...
  searchFiltrados = listaproductos.filter(
    (item) =>
...
const aplicarFiltros = () => {
  ...
  listaproductos.forEach((item) => {
    let bool1 =
      marcasFiltradas.length === 0
        ? true
        : marcasFiltradas.includes(item.marca);
    let bool2 =
      generoFiltrados.length === 0
        ? true
        : generoFiltrados.includes(item.genero);
    let bool3 = false;
    ...
    let bool4 =
      searchFiltrados.length === 0 ? true : searchFiltrados.includes(item);
  ...
  if (productosFiltrados.length === 0) {
    document
      .getElementById("containerArticulos")
      .classList.remove("containerArticulos");
    document
      .getElementById("containerArticulos")
      .classList.add("containerNoProduct");
    document.getElementById(
      "containerArticulos"
    ).innerHTML = `<h2 class="noProduct">NO SE ENCONTRARON PRODUCTOS</h2>`;
  } else {
    document
      .getElementById("containerArticulos")
      .classList.add("containerArticulos");
